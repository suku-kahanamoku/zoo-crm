...
const { data: stats, pending } = useAsyncData("dashboard-stats", async () => {
  ...
  return {
    clients: Number(
      (clients as any)?.meta?.total || (clients as any)?.data?.length || 0,
    ),
    products: Number(
      (products as any)?.meta?.total || (products as any)?.data?.length || 0,
    ),
    categories: Number(
      (categories as any)?.meta?.total ||
  ...
  <div class="mx-auto w-full max-w-7xl space-y-6 px-5 pb-12 pt-6 sm:px-7">
    <section class="crm-hero-panel p-7 sm:p-9">
      <div class="relative z-10 max-w-3xl">
        <p
          class="mb-3 text-xs font-extrabold uppercase tracking-[0.2em] text-emerald-100"
        >
          {{ t("$.dashboard.eyebrow") }}
        </p>
        ...
        <p
          class="mt-4 max-w-2xl text-base leading-7 text-emerald-50/85 sm:text-lg"
        >
          {{ t("$.dashboard.description") }}
        </p>
        <div class="mt-7 flex flex-wrap gap-3">
          <UButton
            :to="localePath('/admin/clients/create')"
            color="secondary"
            size="lg"
      ...
      <UCard class="overflow-hidden">
        <template #header>
          <div class="flex items-center justify-between gap-3">
            ...
            <UButton
              ...
              variant="ghost"
              trailing-icon="i-heroicons-arrow-right"
              >{{ t("$.dashboard.all_customers") }}</UButton
            >
          </div>
        </template>
        <div
          v-if="stats?.recentClients?.length"
        ...
        >
          <NuxtLink
            ...
            :key="client.id"
            :to="
              localePath(client.gen_data?.url || `/admin/clients/${client.id}`)
            "
            class="flex items-center gap-4 py-3.5 first:pt-0 last:pb-0"
          >
            <span
              class="grid size-11 shrink-0 place-items-center rounded-2xl bg-primary/10 font-extrabold text-primary"
            >
      ...
      <UCard>
        ...
        <div v-if="stats?.recentProducts?.length" class="space-y-3">
          ...
          >
            ...
            <span
              class="whitespace-nowrap text-sm font-extrabold text-primary"
              >{{ money(Number(product.price || 0)) }}</span
            >
          </NuxtLink>
        </div>
        <p v-else class="py-6 text-center text-muted">
