...
const { config, response, loading, onSave } = useAdminResource<IClient>(configSource, "admin_clients");
...
<template>
  <div v-if="config" class="mx-auto w-full max-w-5xl px-5 pb-10">
    <div class="crm-page-heading"><div><p class="crm-eyebrow">{{ t("$.section.customer_profiles") }}</p><h1 class="crm-page-title">{{ t('$.admin.edit_client') }}</h1></div></div>
