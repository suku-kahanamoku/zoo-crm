...
const {
  ...
  response,
  loading: pending,
} = useAdminResource<IClient>(configSource, "admin_clients");
      ...
      <UCard>
        <div class="grid gap-6 md:grid-cols-[1.2fr_2fr]">
          <div class="rounded-xl bg-elevated p-5">
            <div
              class="mb-4 flex size-16 items-center justify-center rounded-full bg-primary/10 text-2xl font-black text-primary"
            >
              {{ client.first_name.charAt(0) }}{{ client.last_name.charAt(0) }}
            </div>
            <h2 class="text-xl font-bold">{{ fullName }}</h2>
            <p class="text-muted">{{ client.email }}</p>
            <p v-if="client.phone" class="mt-1 text-muted">
              {{ client.phone }}
            </p>
