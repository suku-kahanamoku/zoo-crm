...
definePageMeta({ layout: "admin", syscode: "admin_products_detail", title: "$.admin.edit_product" });
const { t } = useLang();
const { success, error: toastError } = useToastify();
const { config, response, loading, onSave, goToList, refresh } = useAdminResource<IProduct>(configSource, "admin_products");
...
async function submit(body: Record<string, any>) {
  if (product.value && (await onSave(body, product.value))?.data) goToList();
}
          ...
          <div v-if="product.files?.length">
            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
              ...
              >
                ...
                <p class="truncate p-2 text-xs text-slate-600 dark:text-slate-300">{{ file.name }}</p>
              </a>
            </div>
          </div>
        </div>
